import{u as r,f as h,s as g}from"./assets/footer-46b6302e.js";import{a as u}from"./assets/vendor-2dcf4ad5.js";async function m(){const t="https://food-boutique.b.goit.study/api/products/popular";try{return(await u.get(`${t}`)).data}catch(s){return s.message}}async function f(){const t="https://food-boutique.b.goit.study/api/products/discount";try{return(await u.get(`${t}`)).data}catch(s){return s.message}}const i="/project-ptnpnh24js/assets/sprite-fe336fca.svg",l=document.querySelector(".popular-list");async function b(){try{return await m()}catch(t){console.log(t)}}function v(t){const s=t.map(({_id:c,img:o,name:e,category:a,size:n,popularity:p})=>`<li class="popular-item" data-id='${c}'>
    ...
    </li>`);l.insertAdjacentHTML("beforeend",s.join(""))}function k(t){const s=t.target.closest(".popularbtn-basket");if(!s)return;const o=t.target.closest(".popular-item").dataset.id,e=S();if(e.includes(o))return;const a=s.querySelector(".icon-check");s.querySelector(".icon-shop").classList.add("is-hidden"),a.classList.remove("is-hidden"),e.push(o),y(e),r()}function S(){const t=localStorage.getItem("basket");return t?JSON.parse(t):[]}function y(t){localStorage.setItem("basket",JSON.stringify(t))}const d=document.querySelector(".discount-list");async function $(){try{const t=await f(),s=L(t,2);d.insertAdjacentHTML("beforeend",w(s))}catch(t){console.log(t.message)}}function L(t,s){return t.sort(()=>.5-Math.random()).slice(0,s)}function w(t){return t.map(({name:s,img:c,price:o,_id:e})=>`
