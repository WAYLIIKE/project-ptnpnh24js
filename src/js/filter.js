...
// console.log(categorySelect)
  ...
  try {
    ...
    const optionsHTML = `<option value="" selected hidden>Categories</option>` + categories.map(category => `<option value="${category}">${category.replace(/_/g, ' ')}</option>`
      ).join('') + `<option value="all">Show all</option>`;
